#ifndef _FILO2_THREETWOEXCHANGE_HPP_
#define _FILO2_THREETWOEXCHANGE_HPP_

#include "AbstractOperator.hpp"
// 实现车辆路径问题(VRP)中的3-2交换邻域操作：交换iPrevPrev->iPrev->i 和jPrevPrev->jPrev
namespace cobra {

    class ThreeTwoExchange : public AbstractOperator {
    public:
        ThreeTwoExchange(const Instance &instance_, MoveGenerators &moves_, double tolerance_)
    ...
    protected:
        ...
        inline double compute_cost(const Solution &solution, const MoveGenerator &move) override {
            ...
            const auto jSequenceRem = -solution.get_cost_prev_vertex(jRoute, jPrevPrev) - solution.get_cost_prev_vertex(jRoute, j);
        ...
        inline Cache12 prepare_cache12(const Solution &solution, int vertex) {
            ...
            const auto c_prevprevprev_prevprev = solution.get_cost_prev_vertex(route, c.prevprev);
            c.seqrem1 = -c_prevprevprev_prevprev - solution.get_cost_prev_vertex(route, c.next);
            c.seqrem2 = -c_prevprevprev_prevprev - solution.get_cost_prev_customer(c.v);
        ...
        inline Cache12 prepare_cache12(const Solution &solution, int vertex, int backup) {
            ...
            const auto c_prevprevprev_prevprev = solution.get_cost_prev_vertex(route, c.prevprev);
            c.seqrem1 = -c_prevprevprev_prevprev - solution.get_cost_prev_customer(c.next);
            c.seqrem2 = -c_prevprevprev_prevprev - solution.get_cost_prev_depot(route);
        ...
        inline std::pair<double, double> compute_cost_pair(const MoveGenerator &move, const struct Cache12 i, const struct Cache12 j) {
            ...
            const auto delta1 = this->instance.get_cost(j.prevprevprev, i.prevprev) + c_iv_jv +
                                this->instance.get_cost(i.prevprevprev, j.prevprev) + this->instance.get_cost(j.prev, i.next) + i.seqrem1 +
                                j.seqrem2;
            const auto delta2 = this->instance.get_cost(i.prevprevprev, j.prevprev) + c_iv_jv +
                                this->instance.get_cost(j.prevprevprev, i.prevprev) + this->instance.get_cost(i.prev, j.next) + j.seqrem1 +
                                i.seqrem2;
        ...
        inline double compute_cost(const MoveGenerator &move, const C1 i, const C2 j) {
            ...
            const auto delta = this->instance.get_cost(j.prevprevprev, i.prevprev) + c_iv_jv +
                               this->instance.get_cost(i.prevprevprev, j.prevprev) + this->instance.get_cost(j.prev, i.next) + i.seqrem1 +
                               j.seqrem2;
