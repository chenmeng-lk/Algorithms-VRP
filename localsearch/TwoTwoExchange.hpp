#ifndef _FILO2_TWOTWOEXCHANGE_HPP_
#define _FILO2_TWOTWOEXCHANGE_HPP_

#include "AbstractOperator.hpp"
// 实现车辆路径问题(VRP)中的2-2交换邻域操作：交换i->iPrev和jPrevPrev->jPrev
namespace cobra {

    class TwoTwoExchange : public AbstractOperator {
    public:
        TwoTwoExchange(const Instance &instance_, MoveGenerators &moves_, double tolerance_)
    ...
    protected:
        ...
        inline Cache12 prepare_cache12(const Solution &solution, int vertex) {
            ...
            c.seqrem1 = -solution.get_cost_prev_vertex(route, c.prev) - solution.get_cost_prev_vertex(route, c.next);
            c.seqrem2 = -solution.get_cost_prev_vertex(route, c.prevprev) - solution.get_cost_prev_customer(c.v);
        ...
        inline Cache12 prepare_cache12(const Solution &solution, int vertex, int backup) {
            ...
            c.seqrem1 = -solution.get_cost_prev_customer(c.prev) - solution.get_cost_prev_customer(c.next);
            c.seqrem2 = -solution.get_cost_prev_vertex(route, c.prevprev) - solution.get_cost_prev_depot(route);
        ...
        inline double compute_cost(const MoveGenerator &move, const C1 i, const C2 j) {
            ...
            const auto iSequenceAdd = +this->instance.get_cost(j.prevprevprev, i.prev) + this->moves.get_edge_cost(move);
