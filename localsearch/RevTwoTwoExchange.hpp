#ifndef _FILO2_REVTWOTWOEXCHANGE_HPP_
#define _FILO2_REVTWOTWOEXCHANGE_HPP_

#include "AbstractOperator.hpp"
// 实现车辆路径问题(VRP)中的反向2-2交换邻域操作：交换iPrev->i 和 jNext->jNextNext，并反转iPrev->i，后者是否反转取决于reverse_both_strings
namespace cobra {

    template <bool reverse_both_strings = false>//这是一个模板参数，用于控制是否同时反转jNext->jNextNext
    class RevTwoTwoExchange : public AbstractOperator {
    public:
        RevTwoTwoExchange(const Instance &instance_, MoveGenerators &moves_, double tolerance_)
    ...
    protected:
        ...
        inline Cache12 prepare_cache12(const Solution &solution, int vertex) {
            ...
            c.seqrem1 = -solution.get_cost_prev_vertex(route, c.prev) - c_v_next;
            c.seqrem2 = -c_v_next - solution.get_cost_prev_vertex(route, c.nextnextnext);
        ...
        inline Cache12 prepare_cache12(const Solution &solution, int vertex, int backup) {
            ...
            c.seqrem1 = -solution.get_cost_prev_customer(c.prev) - c_v_next;
            c.seqrem2 = -c_v_next - solution.get_cost_prev_vertex(route, c.nextnextnext);
        ...
        inline std::pair<double, double> compute_cost_pair(const MoveGenerator &move, const struct Cache12 i, const struct Cache12 j) {
            ...
            double jSequenceAdd1, iSequenceAdd2;
            if constexpr (reverse_both_strings) {
