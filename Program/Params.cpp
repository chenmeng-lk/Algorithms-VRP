	  ...
	  vehicleCapacity(vehicleCapacity), timeCost(dist_mtx), verbose(verbose)
{
	...
	*/
	...
	// Initialize RNG
	ran.seed(ap.seed);
	// 根据 AlgorithmParameters 的 seed 初始化快速线性同余随机数生成器（minstd_rand），保证可复现性

	// check if valid coordinates are provided
	// 判断是否为每个节点都提供了坐标信息；部分运行（如不使用 swap*）可能不需要坐标
	...
	for (int i = 0; i <= nbClients; i++)
	{
		...
		// 当启用 swap* 邻域时，需要坐标信息计算极角（polarAngle）以及利用 CircleSector 做几何过滤
	...
	{
		// 若用户未指定车队规模，使用基于总需求/容量的上界估计并留有安全余量
	...
	for (int i = 1; i <= nbClients; i++)
	{
		...
		for (int j = 0; j < std::min<int>(ap.nbGranular, nbClients - 1); j++)//最多20个最近邻居
		{
	...
	// 设置惩罚的初始值，penaltyCapacity 与距离/需求比例相关，保证在合理范围内（0.1..1000）以避免数值不稳定
